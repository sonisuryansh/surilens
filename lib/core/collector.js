...
class SuriCollector extends EventEmitter {
  ...
  startTrace(traceContext) {
    ...
    const trace = {
      ...
      execution: ['Express'],
      steps: [
        { node: 'Express', spanId: traceContext.rootSpanId || traceContext.activeSpanId || 'sp_root', parentSpanId: null, category: 'express', timestamp: now, elapsedMs: 0 }
      ],
      logs: [],
      stagePayloads: {
  ...
  transitionNode(traceId, targetNode, metadata = {}) {
    ...
    const parentSpanId = trace.activeSpanId || trace.rootSpanId || null;
    const spanId = 'sp_' + Math.random().toString(36).substring(2, 10);
    trace.activeSpanId = spanId;

    // Resolve runtime category from execution metadata
    const lower = targetNode.toLowerCase();
    let category = metadata.category || metadata.type || 'function';
    if (lower.includes('express')) category = 'express';
    else if (lower.includes('router')) category = 'router';
    else if (lower.includes('middleware')) category = 'middleware';
    else if (lower.includes('mongo') || lower.includes('prisma') || lower.includes('sequelize') || lower.includes('postgres') || lower.includes('mysql') || lower.includes('database')) category = 'database';
    else if (lower.includes('redis') || lower.includes('cache')) category = 'redis';
    else if (lower.includes('jwt') || lower.includes('bcrypt') || lower.includes('auth')) category = 'jwt';
    else if (lower.includes('external') || lower.includes('axios') || lower.includes('fetch')) category = 'external_http';
    else if (lower === 'client') category = 'client';
    else if (lower === 'response') category = 'response';
  ...
  completeTrace(traceId, statusCode = 200, error = null, responseBody = null) {
    ...
    trace.endTime = now;
    trace.error = error ? { message: error.message, stack: error.stack } : (statusCode >= 500 ? { message: `HTTP ${statusCode} Server Error` } : null);
