...
const { getCallerInfo } = require('./async-context');

/**
 * Manual & Dynamic Step Helper for custom demarcation.
 * Automatically resolves caller file, folder, and function name if unspecified.
 */
function recordStep(nodeName, metadata = {}) {
  const context = getContext();
  if (!context) return;

  let resolvedNode = nodeName;
  const caller = getCallerInfo();

  if (!resolvedNode && caller) {
    const fName = (caller.folderName || '').toLowerCase();
    if (['controllers', 'services', 'repositories', 'utils', 'helpers', 'database', 'middleware'].includes(fName)) {
      const titleFolder = fName.charAt(0).toUpperCase() + fName.slice(0, -1);
      resolvedNode = `${titleFolder} (${caller.baseName})`;
    } else {
      resolvedNode = caller.baseName || caller.funcName || 'Step';
    }
  }

  collector.transitionNode(context.traceId, resolvedNode || 'Step', {
    caller,
    ...metadata
  });
}
